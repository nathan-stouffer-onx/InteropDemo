...
cmake -S . -B build -DTARGET_ARCHITECTURE=arm64-ios -DCMAKE_CXX_COMPILER_TARGET=arm64-ios -DCMAKE_OSX_SYSROOT=${DEVICE_SYSROOT}
make -C build install

# configure and build for x64 ios
cmake -S . -B build -DTARGET_ARCHITECTURE=x64-ios -DCMAKE_CXX_COMPILER_TARGET=x64-ios -DCMAKE_OSX_SYSROOT=${SIMULATOR_SYSROOT}
make -C build install